...
class LLMSettings(ExtendedBaseSettings):
    # LLM prompt/response logging (under log/*/llm_messages/).
    log_llm_chat_to_file: bool = True
    log_llm_chat_to_console: bool = False
    # Deprecated: if set, overrides both flags above (LOG_LLM_CHAT_CONTENT in .env).
    log_llm_chat_content: bool | None = None

    @model_validator(mode="after")
    def _apply_legacy_llm_log_flags(self) -> Self:
        if self.log_llm_chat_content is not None:
            self.log_llm_chat_to_file = self.log_llm_chat_content
            self.log_llm_chat_to_console = self.log_llm_chat_content
        return self
