...
from alphapilot.core.utils import LLM_CACHE_SEED_GEN, SingletonBaseClass
from alphapilot.log import LogColors
...
class APIBackend:
    ...
    def _log_llm(self, msg: str, *, raw: bool = False) -> None:
        """Log LLM prompts/responses to trace files and/or console."""
        if LLM_SETTINGS.log_llm_chat_to_file:
            logger.info_file_only(msg, tag="llm_messages", raw=raw)
        if LLM_SETTINGS.log_llm_chat_to_console:
            logger.info(msg, tag="llm_messages", raw=raw)
    ...
    def _create_chat_completion_inner_function(  # noqa: C901, PLR0912, PLR0915
        ...
        *,
        ...
        if LLM_SETTINGS.log_llm_chat_to_file or LLM_SETTINGS.log_llm_chat_to_console:
            self._log_llm(self._build_log_messages(messages))
        ...
        if self.use_chat_cache:
            ...
            if cache_result is not None:
                if LLM_SETTINGS.log_llm_chat_to_file or LLM_SETTINGS.log_llm_chat_to_console:
                    self._log_llm(f"{LogColors.CYAN}Response:{cache_result}{LogColors.END}")
        ...
        if self.use_llama2:
            ...
            if LLM_SETTINGS.log_llm_chat_to_file or LLM_SETTINGS.log_llm_chat_to_console:
                self._log_llm(f"{LogColors.CYAN}Response:{resp}{LogColors.END}")
            ...
            if self.chat_stream:
                ...
                log_stream = LLM_SETTINGS.log_llm_chat_to_file or LLM_SETTINGS.log_llm_chat_to_console
                if log_stream:
                    self._log_llm(f"{LogColors.CYAN}Response:{LogColors.END}")
                ...
                for chunk in response:
                    ...
                    if log_stream:
                        self._log_llm(LogColors.CYAN + content + LogColors.END, raw=True)
                    resp += content
