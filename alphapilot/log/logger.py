...
from alphapilot.core.utils import SingletonBaseClass

from .storage import FileStorage
from .utils import LogColors, get_caller_info


class AgentLog(SingletonBaseClass):
    """
    ...
    def info_file_only(self, msg: str, *, tag: str = "", raw: bool = False) -> None:
        """Append a log line to the trace file without printing to the console."""
        caller_info = get_caller_info()
        tag = f"{self._tag}.{tag}.{self.get_pids()}".strip(".")
        log_file_path = self.log_trace_path / tag.replace(".", "/") / "common_logs.log"
        log_file_path.parent.mkdir(parents=True, exist_ok=True)
        clean_msg = LogColors.remove_ansi_codes(msg)
        if raw:
            line = clean_msg if clean_msg.endswith("\n") else f"{clean_msg}\n"
        else:
            ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
            caller = (
                f"{caller_info.get('name', '')}:{caller_info.get('function', '')}:"
                f"{caller_info.get('line', '')}"
            )
            line = f"{ts} | INFO     | {caller} - {clean_msg}\n"
