...
from alphapilot.core.knowledge_base import KnowledgeBase
...
class KnowledgeMetaData:
    ...
    def split_into_trunk(self, size: int = 1000, overlap: int = 0):
        ...
        self.trunks_embedding = get_llm().create_embedding(input_content=self.trunks)
    ...
    def create_embedding(self):
        ...
        if self.embedding is None:
            self.embedding = get_llm().create_embedding(input_content=self.content)
...
def contents_to_documents(contents: List[str], label: str = None) -> List[Document]:
    ...
    for i in range(0, len(contents), size):
        embedding.extend(get_llm().create_embedding(input_content=contents[i : i + size]))
