...
from alphapilot.components.document_reader.document_reader import (
...
from alphapilot.modules.alpha_mining.qlib.factor_experiment_loader.json_loader import (
...
def __extract_factors_name_and_desc_from_content(
    ...
    session = get_llm().chat_session(
        document_process_prompts["extract_factors_system"],
    )
...
def __extract_factors_formulation_from_content(
    ...
    session = get_llm().chat_session(system_prompt)
...
def __check_factor_dict_relevance(
    ...
    extract_result_resp = get_llm().chat_completion(
...
def __check_factor_duplication_simulate_json_mode(
    ...
    for current_df in final_list:
        ...
        session = get_llm().chat_session(
            document_process_prompts["factor_duplicate_system"],
        )
        for _ in range(10):
            extract_result_resp = session.build_chat_completion(
...
"""
    ...
    embeddings = get_llm().create_embedding(full_str_list)
