"""Streamlit dashboard for AlphaPilot Qlib backtest artifacts."""
...
from alphapilot.app.backtest_viewer.loader import (
    DEFAULT_LOG_ROOT,
...
def main() -> None:
    ...
    if not workspaces:
        st.warning(f"在 `{root}` 下未找到含 ret.pkl 的回测 workspace。请先成功运行 `alphapilot mine` 或 `alphapilot backtest`。")
        st.stop()
    ...
    st.title(f"📊 AlphaPilot 回测详情{title_suffix}")
    st.caption(
