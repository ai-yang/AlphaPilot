...
from alphaagent.app.data.adjust_prices import apply_adjust_directory
from alphaagent.app.data.generate_h5 import generate_daily_pv_h5
from alphaagent.app.data.prepare_cn import (
    DEFAULT_STOCK_CSV,
...
from alphaagent.app.data.qlib_convert import (
    DEFAULT_INCLUDE_FIELDS,
    DEFAULT_QLIB_DIR,
    dump_csv_to_qlib,
    extend_future_calendar,
)
from alphaagent.app.data.stock_list import (
    default_market_name,
...
class PrepareDataCLI:
    ...
    def download(
        ...
        行情 CSV 支持增量。除权模式下每只股票行情保存后会立即下载全历史复权因子（自 1990 年起）；
        若中途中断，已完成的股票应有对应因子文件，其余可再跑 refresh_factors。

        推荐流程::

            alphaagent prepare_data download --stock_csv pools.csv --adjust_mode none
            alphaagent prepare_data apply_adjust --adjust_mode forward
            alphaagent prepare_data convert --data_path ~/.qlib/.../raw_data_forward_adjust --market pools
        """
    ...
    def refresh_factors(
        ...
        if all_market:
            ...
            from alphaagent.app.data.prepare_cn import get_all_stocks_in_period
        ...
        if refresh_factors_if_needed:
            ...
            if incomplete:
                msg = (
                    ...
                    "请先运行: alphaagent prepare_data refresh_factors --stock_csv <你的列表>"
                )
                if refresh_factors_if_needed:
