...
from alphaagent.app.data.qlib_dump.dump_bin import DumpDataAll
from alphaagent.app.data.qlib_dump.future_calendar_collector import run as collect_future_calendar
...
def dump_csv_to_qlib(
    ...
    if not data_path.is_dir() or not any(data_path.glob("*.csv")):
        raise FileNotFoundError(
            f"未在 {data_path} 找到 CSV 文件。请先运行: alphaagent prepare_data download"
        )
...
def extend_future_calendar(
    ...
    if not calendar_path.exists():
        raise FileNotFoundError(
            f"交易日历不存在: {calendar_path}。请先运行: alphaagent prepare_data dump"
        )
