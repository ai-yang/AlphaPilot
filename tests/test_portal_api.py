...
def test_portal_file_roots_reject_unconfigured_paths(tmp_path, monkeypatch) -> None:  # noqa: ANN001
    ...
    assert c.get("/api/market/symbols", params={"data_dir": str(outside)}).status_code == 410
    assert c.get(
        "/api/market/kline", params={"data_dir": str(outside), "symbol": "secret"}
    ).status_code == 410
    assert c.get("/api/backtests", params={"workspace_root": str(outside)}).status_code == 410
    assert c.get("/api/backtests/leaderboards", params={"workspace_root": str(outside)}).status_code == 410
    assert c.get(
...
def test_notify_command_parse_auth_and_dispatch(tmp_path, monkeypatch) -> None:  # noqa: ANN001
    ...
    from alphapilot.research import channels
    monkeypatch.setattr(channels, "command_spec", lambda kind, kwargs, *_: {"kind": kind, "input": kwargs})
    monkeypatch.setattr(channels.TaskService, "submit", lambda self, spec, actor, key: fake_start_job(spec["kind"], spec["input"]))
...
def test_notify_natural_language_confirm_and_api_masking(tmp_path, monkeypatch) -> None:  # noqa: ANN001
    ...
    from alphapilot.research import channels
    monkeypatch.setattr(channels, "command_spec", lambda kind, kwargs, *_: {"kind": kind, "input": kwargs})
    monkeypatch.setattr(channels.TaskService, "submit", lambda self, spec, actor, key: started.append((spec["kind"], spec["input"])) or {"job_id": "job2", "status": "queued"})
...
def test_removed_timing_routes_and_job_kind_are_rejected(tmp_path, monkeypatch) -> None:  # noqa: ANN001
    ...
    assert removed_job.status_code == 410
    assert removed_job.json()["code"] == "API_REMOVED"
